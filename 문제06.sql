...
select
    trans_type as "교통수단명",
    sum(total_acct_num) as "교통 사고 발생 수",
    sum(death_person_num) as "총 사망자 수",
    round(sum(death_person_num)/sum(total_acct_num),3) as "사건 당 평균 사망자 수"
...
select 
    count(decode(breath, 'gill' , 1)) as "변온, 아가미 호흡",
    count(decode(breath, 'lung', 1)) as "변온, 폐 호흡"
from tblZoo
    group by thermo
